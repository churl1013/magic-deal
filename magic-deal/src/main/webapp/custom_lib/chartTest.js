...
var initChart = function(data) {
	...
	nodes = chartWrap.selectAll("div").data(data).enter().append("div")
					...
					.style("width", function(d) {
						return qualityScale(d.quality) + "px";
					})
					.style("background-image", function(d) {
						return "url('../img/cimg/" + d.img + "')";
					})
					.style("height", function(d) {
						return qualityScale(d.quality) + "px";
					})
					.style("left", function(d) {
						return (cWidth / 2) - (qualityScale(d.quality) / 2) + "px";
					})
					.style("top", function(d) {
						return (cHeight / 2) - (qualityScale(d.quality) / 2) + "px";
					})
					.style("opacity", "0")
					.on("click", function(d) {
...
var nodeRightClickEvt = function(d, obj) {
	...
	var newBoxtxt = "<div data-idx='"+d.key+"' class='favoriteBox'>";
	...
	newBoxtxt += '<div class="ui black icon button"><i class="zoom icon"></i>Detail</div>';
	newBoxtxt += '</div>';
	newBoxtxt += '</div>';
	newBoxtxt += '</div>';
	newBoxtxt += "<img src='../img/cimg/"+d.img+"' class='favorite-img'/>";
	newBoxtxt += '<div class="ui inverted statistic">';
	newBoxtxt += '<div class="text value">';
	newBoxtxt += '<img src="../img/sample-test/profile-exam.JPG" class="ui circular inline image"><br/>';
	newBoxtxt += d.price;
	newBoxtxt += '</div><div class="label">';
	newBoxtxt += d.productName;
	newBoxtxt += '</div></div>';
	newBoxtxt += "</div></div>";
...
var addfilterList = function(list) {
	list.each(function(d) {
		...
		appendHtml += "</i></span>";
		appendHtml += "&nbsp;&nbsp;" + d.productName;
		nBox.html(appendHtml);
		nBox.attr("data-fidx", d.key);
		$("#filterList").append(nBox);
		
		filterList["f"+d.key] = {};
		filterList["f"+d.key].obj = this;
		filterList["f"+d.key].interval = null;
		
		filterList.length++;
		nBox.on("click", function() {
			var idx = $(this).attr("data-fidx");
			var obj = filterList["f"+d.key].obj;
...
var distanceAxisDraw = function(before, limit, delay) {
	...
	filterNode = nodes.filter(function(d) {
		var flag = d.distance<=limit && d.distance>before;
		return flag;
	});
...
var priceAxisDraw = function() {
	...
	// 최대 - 평균 사이 축
	priceAxisTick((avr+maxPrice)/2, size);
	// 최소 - 평균 사이 축
	priceAxisTick((avr+minPrice)/2, size);
...
var getCircleQualitySize = function(flag, d) {
	...
	if(flag) {
		// 퀄리티 적용 사이즈
		size = qualityScale(d.quality) + "px";
	}
	return size;
}
...
(function() {
	...
	d3.json("../custom_lib/product.json", initChart);
})();
